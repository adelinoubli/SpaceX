...
//
//   let launche = try? JSONDecoder().decode(Launche.self, from: jsonData)

import Foundation

typealias Launches = [LauncheElement]

// MARK: - LauncheElement
struct LauncheElement: Codable {
...
// To parse the JSON, add this file to your project and do:
//
//   let crew = try? newJSONDecoder().decode(Crew.self, from: jsonData)
...
// To parse the JSON, add this file to your project and do:
//
//   let patch = try? newJSONDecoder().decode(Patch.self, from: jsonData)
...
typealias Launche = [LauncheElement]
